...
class TurboQuantVectorStore(VectorStore):
    ...
    **Thread safety.** The store is safe for concurrent multi-threaded
    ...
    # holding between 1 and 999 vectors leaves the copy's index committed
    # to ``"identity"`` calibration for good, while the original keeps its
    ...
    # a copy of an *emptied* store — deleting every id leaves nothing
    # encoded under identity, so the copy stays warming up (#418).
