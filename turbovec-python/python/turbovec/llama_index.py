...
class TurboQuantVectorStore(BasePydanticVectorStore):
    ...
    **Thread safety.** The store is safe for concurrent multi-threaded
    ...
    # copy of a store emptied *while warming up* — ``delete_nodes`` of
    # every id leaves nothing encoded under identity, so the copy comes
    # back ``"warming_up"`` and can still fit (#418).
